...
export const companiesAPI = {
  getAll: async (): Promise<ApiResponse<Company[]>> => {
    console.log('[companiesAPI] → GET /companies');
    const result = await apiRequest<Company[]>('/companies', { method: 'GET' });
    if (result.success) {
      console.log('[companiesAPI] ✅ received:', Array.isArray(result.data) ? result.data.length : '(not array)', 'companies', result.data);
    } else {
      console.error('[companiesAPI] ❌ failed:', result.error);
    }
    return result;
  },
...
// ─────────────────────────────────────────────────────────────────────────────
// Alias so add_user_popup.tsx import { usersAPI } keeps working
export const usersAPI = portalUsersAPI;
