...
export function useCourseCatalog({
  ...
  toast,
  onOpenCourse,
}: Pick<CourseCatalogProps, "courses" | "setCourses" | "categories" | "setCategories" | "toast" | "onOpenCourse">) {
  ...
  useEffect(() => {
    api.settings.getCategories().then(res => {
      if (res.success && Array.isArray(res.data) && res.data.length > 0) {
        setCategories(res.data);
      }
    }).catch(() => {
      // Non-fatal — keeps whatever default categories are already in state
    });
  }, []);
  ...
  // Modules
  const handleModSave = async (idx: number, modules: Module[], onDone?: (success: boolean) => void) => {
    const course = courses[idx];
    console.group('[handleModSave] 💾 Save triggered');
    console.log('[handleModSave] course idx:', idx);
    console.log('[handleModSave] course id:', course?.id);
    console.log('[handleModSave] course title:', course?.title);
    console.log('[handleModSave] modules to save:', modules.length, modules);
    // Update state immediately so UI reflects changes
    setCourses(prev => prev.map((c, i) => i === idx ? { ...c, modules } : c));
    try {
      if (!course.id) {
        console.warn('[handleModSave] ⚠️ no course.id — saving locally only');
        toast('Modules saved locally (no server ID)');
        onDone?.(true);
        console.groupEnd();
        return;
      }
      console.log('[handleModSave] 📡 calling api.courses.updateModules(' + course.id + ', modules)');
      const res = await api.courses.updateModules(course.id, modules);
      console.log('[handleModSave] 📥 response:', res);
      if (res.success) {
        console.log('[handleModSave] ✅ saved successfully');
        onDone?.(true);
      } else {
        console.error('[handleModSave] ❌ server error:', res.error);
        toast(`Error: ${res.error || 'Failed to save modules'}`);
        onDone?.(false);
      }
    } catch (err) {
      console.error('[handleModSave] ❌ exception thrown:', err);
