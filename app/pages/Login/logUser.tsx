/* ==============================================================
   ...
   Auth is now done against the real database via:
     POST /api/auth/login  { email, password }
   access_level mapping:
     super_admin | system_admin  →  role "admin"  → DashboardAdmin
     manager     | user          →  role "client" → OverviewPage
   ============================================================== */
...
export interface UserProfile {
  id:              number;
  username:        string;   // email — kept for backward-compat
  role:            UserRole;
  accessLevel:     string;   // raw DB value: super_admin | system_admin | manager | user
  fullName:        string;
  initials:        string;
  position:        string;
  company:         string;
  ...
  backgroundColor: string | null;   // ← company.background_color
  panelColor:      string | null;   // ← company.panel_color
}
...
export default function LoginAdmin({ onLoginSuccess }: LoginAdminProps) {
  ...
  /* ── Sign-in handler — calls the real API ─────────────── */
  const handleSignIn = useCallback(async () => {
    ...
    try {
      // FIX: Prime the XSRF-TOKEN cookie before the POST so Laravel doesn't 419
      await getCsrfCookie();
      ...
      const res = await fetch(`${API_BASE}/api/auth/login`, {
        ...
        headers: {
          ...
          "X-XSRF-TOKEN":  decodeURIComponent(xsrf), // FIX: Laravel reads this header
        },
      ...
      /* Build the profile object from the API response */
      const u = data.user;
      const profile: UserProfile = {
        ...
        companyLogo:     u.company_logo     ?? null,
        profilePhoto:    u.profile_photo    ?? null,
        backgroundColor: u.background_color ?? null,   // ← from company table
        panelColor:      u.panel_color      ?? null,   // ← from company table
      };
  ...
  // FIX: depend on handleSignIn so the keydown listener always has the latest closure
  useEffect(() => {
    ...
    <>
      ...
      <div className="fixed inset-0 overflow-hidden"
        ...
        {[
          ...
          <div key={i} className="absolute rounded-full opacity-55"
            style={{ width:b.sz, height:b.sz, background:`radial-gradient(circle,${b.c},transparent)`, filter:"blur(80px)", animation:`gxBlobDrift ${b.dur} ease-in-out infinite alternate`, ...b.pos }} />
        ))}
        <div className="absolute inset-0"
             style={{ backgroundImage:`linear-gradient(rgba(124,58,237,0.06) 1px,transparent 1px),linear-gradient(90deg,rgba(14,165,233,0.05) 1px,transparent 1px)`, backgroundSize:"60px 60px" }} />
      ...
      <div className="fixed inset-0 z-[100] overflow-y-auto overflow-x-hidden"
        ...
        <div className="flex min-h-full items-center justify-center"
          style={{ padding: isMobile ? `24px ${Math.round(tok.cardPadX * 0.45)}px 40px` : `${Math.round(tok.cardPadY * 0.55)}px 20px` }}>
          <div ref={cardWrapperRef} className="w-full shrink-0" style={{ maxWidth: tok.cardMaxW }}>

            <div id="loginCard" className="gx-card-in relative overflow-hidden w-full backdrop-blur-2xl"
              style={{
              ...
              {!isVeryShort && (
                <div className="gx-fade-up-1 flex items-center gap-3 overflow-hidden"
                     style={{ height: tok.logoRowH, marginBottom: tok.logoRowMb }}>
                  <img src="/img_assets/genieX_branding.png" alt="GenieX Logo" className="object-contain block shrink-0"
                    style={{ width: tok.logoW, height: tok.logoH, marginTop: -tok.logoNudge, marginBottom: -tok.logoNudge }} />
                  ...
                  <span className="font-medium tracking-[0.15em] uppercase"
              ...
              <div className="gx-fade-up-2" style={{ marginTop: isVeryShort ? 0 : tok.headingMt, marginBottom: tok.headingMb }}>
                <h1 className="font-light leading-none m-0 mb-1.5"
                    style={{ fontFamily:"'Cormorant Garamond',serif", fontSize: tok.h1, color:"#0f0730" }}>
                  Welcome{" "}<em className="italic font-light" style={{ color:"#0284c7" }}>Back.</em>
                </h1>
                <p className="m-0 font-normal tracking-wide"
                   style={{ fontFamily:"'Jost',sans-serif", fontSize: tok.sub, marginTop: 6, marginBottom: Math.round(tok.headingMb * 0.5), color:"#3b2270" }}>
              ...
              <div className="gx-fade-up-3">
                <label className="block font-semibold tracking-[0.12em] uppercase"
                ...
                <div className="relative" style={{ marginBottom: tok.fieldMb }}>
                  <input type="text" placeholder="Enter your email here...." autoComplete="username"
                    id="usernameInput" ref={usernameInputRef}
                    className="block w-full font-light outline-none transition-all duration-300"
                    style={{
                      fontFamily: "'Jost',sans-serif",
                      paddingTop: tok.inputPy, paddingBottom: tok.inputPy,
                      paddingLeft: tok.inputPx, paddingRight: tok.inputPx,
                      fontSize: tok.inputFs, borderRadius: 10,
                      ...
                      boxShadow: inputFocused === "username" ? "0 0 0 3px rgba(109,40,217,0.12),0 0 20px rgba(109,40,217,0.06)" : "0 1px 0 rgba(255,255,255,0.8) inset",
                      color: "#1e1048", backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)",
                      transform: inputFocused === "username" ? "translateY(-1px)" : "none",
                    }}
                    onFocus={() => setInputFocused("username")} onBlur={() => setInputFocused(null)} />
                  <div className="absolute bottom-0 h-0.5 pointer-events-none transition-all duration-[450ms]"
                    style={{ left: inputFocused === "username" ? "50%" : 0, width: inputFocused === "username" ? 0 : "100%", borderRadius: "0 0 10px 10px", background: "linear-gradient(90deg,#6d28d9,#a78bfa,#0284c7)", transitionTimingFunction: "cubic-bezier(0.4,0,0.2,1)" }} />
                </div>
              </div>
              ...
              <div className="gx-fade-up-4">
                ...
                <div className="relative" style={{ marginBottom: Math.round(tok.fieldMb * 0.6) }}>
                  <input type={showPassword ? "text" : "password"} placeholder="Enter your password here...."
                    id="passwordInput" ref={passInputRef} autoComplete="current-password"
                    className="block w-full font-light outline-none transition-all duration-300"
                    style={{
                      fontFamily: "'Jost',sans-serif",
                      paddingTop: tok.inputPy, paddingBottom: tok.inputPy,
                      paddingLeft: tok.inputPx, paddingRight: tok.inputPx + 30,
                      fontSize: tok.inputFs, borderRadius: 10,
                      ...
                      boxShadow: inputFocused === "password" ? "0 0 0 3px rgba(109,40,217,0.12),0 0 20px rgba(109,40,217,0.06)" : "0 1px 0 rgba(255,255,255,0.8) inset",
                      color: "#1e1048", backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)",
                      transform: inputFocused === "password" ? "translateY(-1px)" : "none",
                    }}
                    onFocus={() => setInputFocused("password")} onBlur={() => setInputFocused(null)} />
                  <div className="absolute bottom-0 h-0.5 pointer-events-none transition-all duration-[450ms]"
                    style={{ left: inputFocused === "password" ? "50%" : 0, width: inputFocused === "password" ? 0 : "100%", borderRadius: "0 0 10px 10px", background: "linear-gradient(90deg,#6d28d9,#a78bfa,#0284c7)", transitionTimingFunction: "cubic-bezier(0.4,0,0.2,1)" }} />
                  <button id="eyeBtn" type="button"
                    ...
                    style={{ right: tok.inputPx - 2, color: showPassword ? "#a78bfa" : "#3b2270", opacity: showPassword ? 1 : 0.6 }}
              ...
              {/* Options row */}
              <div className={`gx-fade-up-5 flex items-center justify-between ${tier === "xs" ? "flex-wrap gap-2" : ""}`}
                   style={{ marginTop: tok.optionsMt }}>
                <label htmlFor="stayCheck" className="flex items-center gap-2 cursor-pointer select-none font-normal"
                  style={{ fontFamily:"'Jost',sans-serif", fontSize: tok.sub - 1, color:"#3b2270" }}>
                  <input type="checkbox" className="hidden" id="stayCheck" checked={stayChecked} onChange={() => setStayChecked(v => !v)} />
                  <div className="relative shrink-0 w-4 h-4 rounded backdrop-blur-sm transition-all duration-200"
                    style={{ border: "1px solid rgba(124,58,237,0.4)", background: stayChecked ? "linear-gradient(135deg,#6d28d9,#0284c7)" : "rgba(255,255,255,0.8)" }}>
                    {stayChecked && (
                ...
                <button type="button" onClick={() => router.push("/pages/Forgot_Password")}
                  onMouseEnter={() => setForgotHover(true)} onMouseLeave={() => setForgotHover(false)}
                  ...
                  Forgot password?
                  <span className="absolute -bottom-px left-0 right-0 h-px transition-transform duration-300 origin-left"
                    style={{ background: "linear-gradient(90deg,#0284c7,#6d28d9)", transform: forgotHover ? "scaleX(1)" : "scaleX(0)" }} />
                </button>
              </div>

              {/* Sign in button */}
              <button ref={signInBtnRef} id="signInBtn" onClick={handleSignIn} disabled={isLoading}
                ...
                style={{
                  fontFamily: "'Jost',sans-serif",
                  marginTop: tok.signinMt, marginBottom: tok.signinMb,
                  padding: `${tok.btnPy}px`, fontSize: tok.btnFs,
              ...
              {!isVeryShort && (
                <div className="gx-fade-up-7 flex items-center justify-center gap-1.5 font-medium tracking-[0.14em] uppercase mb-0.5"
              ...
              <div className="gx-fade-up-8 flex items-center gap-3.5"
                   style={{ marginTop: tok.dividerMy, marginBottom: tok.dividerMy }}>
                <div className="flex-1 h-px" style={{ background:"linear-gradient(to right,transparent,rgba(124,58,237,0.2),transparent)" }} />
              ...
              <div className="gx-fade-up-9 grid grid-cols-2 gap-3">
                {/* Google */}
                <button onMouseEnter={() => setGoogleHover(true)} onMouseLeave={() => setGoogleHover(false)}
                  ...
                  style={{ fontFamily:"'Jost',sans-serif", padding:`${tok.socialPy}px 12px`, fontSize:Math.max(11,tok.sub-1), color:"#0f0730", backdropFilter:"blur(10px)", background:googleHover?"rgba(237,233,254,0.95)":"rgba(255,255,255,0.85)", border:googleHover?"1.5px solid rgba(109,40,217,0.4)":"1.5px solid rgba(109,40,217,0.22)", boxShadow:googleHover?"0 4px 20px rgba(109,40,217,0.18),0 1px 0 rgba(255,255,255,0.9) inset":"0 1px 0 rgba(255,255,255,0.9) inset,0 2px 8px rgba(109,40,217,0.06)", transform:googleHover?"translateY(-1px)":"none" }}>
                ...
                {/* GitHub */}
                <button onMouseEnter={() => setGithubHover(true)} onMouseLeave={() => setGithubHover(false)}
                  className="flex items-center justify-center gap-2 overflow-hidden relative cursor-pointer whitespace-nowrap rounded-[10px] font-medium transition-all duration-200"
                  style={{ fontFamily:"'Jost',sans-serif", padding:`${tok.socialPy}px 12px`, fontSize:Math.max(11,tok.sub-1), color:"#0f0730", backdropFilter:"blur(10px)", background:githubHover?"rgba(237,233,254,0.95)":"rgba(255,255,255,0.85)", border:githubHover?"1.5px solid rgba(109,40,217,0.4)":"1.5px solid rgba(109,40,217,0.22)", boxShadow:githubHover?"0 4px 20px rgba(109,40,217,0.18),0 1px 0 rgba(255,255,255,0.9) inset":"0 1px 0 rgba(255,255,255,0.9) inset,0 2px 8px rgba(109,40,217,0.06)", transform:githubHover?"translateY(-1px)":"none" }}>
      ...
      {/* Toast */}
      <div className="fixed left-1/2 z-[9999] font-medium text-center rounded-2xl transition-all duration-300 ease-in-out"
        style={{ fontFamily:"'Jost',sans-serif", bottom: isMobile ? 20 : 32, transform:`translateX(-50%) translateY(${toastVisible ? 0 : 16}px)`, opacity: toastVisible ? 1 : 0, pointerEvents: toastVisible ? "auto" : "none", background:"rgba(30,20,50,0.92)", backdropFilter:"blur(12px)", border:"1px solid rgba(124,58,237,0.35)", padding: isMobile ? "10px 16px" : "11px 20px", color:"#e9d5ff", fontSize: isMobile ? 12 : 13, boxShadow:"0 8px 32px rgba(0,0,0,0.4)", maxWidth:"calc(100vw - 32px)", wordBreak:"break-word" }}>
      ...
      {/* Error modal */}
      {errorVisible && (
        <div onClick={(e) => { if (e.target === e.currentTarget) closeError(); }}
          ...
          <div className="relative w-full max-w-sm rounded-[20px]"
            style={{ background:"rgba(18,10,38,0.97)", border:"1px solid rgba(124,58,237,0.25)", padding:"28px 24px 24px", boxShadow:"0 20px 60px rgba(0,0,0,0.5),0 0 0 1px rgba(124,58,237,0.1)", animation:"modalPop 0.3s cubic-bezier(0.34,1.56,0.64,1)" }}>
            <button onClick={closeError} onMouseEnter={() => setCloseErrHover(true)} onMouseLeave={() => setCloseErrHover(false)}
              ...
              style={{ background: closeErrHover?"rgba(124,58,237,0.18)":"rgba(124,58,237,0.08)", border: closeErrHover?"1px solid rgba(124,58,237,0.4)":"1px solid rgba(124,58,237,0.15)", color: closeErrHover?"#c4a8ff":"rgba(196,168,255,0.5)" }}>✕</button>
            ...
            <div className="text-[15px] font-bold tracking-[-0.01em] mb-1.5" style={{ color:"#ede9fe" }}>Login Failed</div>
            <p className="text-[13px] leading-relaxed m-0 mb-5" style={{ color:"rgba(196,168,255,0.55)" }}>{errorMsg}</p>
            <button onClick={closeError} onMouseEnter={() => setTryAgainHover(true)} onMouseLeave={() => setTryAgainHover(false)}
              className="block w-full cursor-pointer rounded-xl font-semibold text-[13px] tracking-[0.01em] transition-all duration-150"
              style={{ fontFamily:"'Jost',sans-serif", padding:"11px 0", border: tryAgainHover?"1px solid rgba(124,58,237,0.55)":"1px solid rgba(124,58,237,0.35)", background: tryAgainHover?"linear-gradient(135deg,rgba(124,58,237,0.35) 0%,rgba(91,33,182,0.4) 100%)":"linear-gradient(135deg,rgba(124,58,237,0.2) 0%,rgba(91,33,182,0.25) 100%)", color: tryAgainHover?"#ddd6fe":"#c4a8ff" }}>
              Try Again
            </button>
          </div>
        </div>
