...
/* ==============================================================
   ...
   CHANGE: BranchesTable now fetches implementation_date per branch
           and colours the status dot:
             green (#16a34a) = has implementation_date (Active)
             red   (#dc2626) = null                    (Inactive)
   ============================================================== */
...
import type { DBUser } from './dashboard_overview_users';
...
export const PlusIcon     = () => <svg viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="2" width="10" height="10"><path d="M6 1v10M1 6h10"/></svg>;
...
export function ClientTrendChart({ clientId, period }: { clientId: number; period: TicketPeriod }) {
  ...
  const sliced = { newTickets: sliceTrend(trend.newTickets), resolved: sliceTrend(trend.resolved), critical: sliceTrend(trend.critical) };
    ...
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: '14px 16px 12px', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={summaryBlock}>
        ...
        <div style={summaryText}>
          {totalTracked > 0 ? (<>{totalTracked} total ticket{totalTracked !== 1 ? 's' : ''} on record — <strong style={{ color: C.red }}>{openCount} open</strong>, <strong style={{ color: C.amber }}>{pendingCount} pending</strong>, <strong style={{ color: C.green }}>{closedCount} resolved</strong>.{highPriorityOpen > 0 ? ` ${highPriorityOpen} open ticket${highPriorityOpen !== 1 ? 's' : ''} are high/critical priority — immediate attention needed.` : ' No high-priority open tickets at this time.'}</>) : ('No tickets recorded yet for this client.')}
        </div>
      </div>
    </div>
  );
...
export function ClientCategoryChart({ clientId }: { clientId: number }) {
  ...
  const displayCats = categories.length > 0 ? categories : analytics.categories.map(c => ({ label: c.label, color: c.color, n: c.n, thisWeek: c.n, lastWeek: 0, change: c.change, dir: c.dir }));
    ...
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: '14px 16px 12px', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={summaryBlock}>
        ...
        <div style={summaryText}>
          ...
          {decliningCat ? `${decliningCat.label} tickets are down ${Math.abs(decliningCat.change)}% this week.` : 'All categories trending up or flat.'}
        </div>
      </div>
    </div>
  );
...
/* ════════════════════════════════════════════════════
   HELPER — resolve shared license for Retail/Warehouse
════════════════════════════════════════════════════ */
function resolveSharedLicense(client: Client): string {
  if (client.licenseId?.trim()) return client.licenseId.trim();
  if (client.branchLicenses) {
    const first = Object.values(client.branchLicenses).find(v => v?.trim());
...
/* ════════════════════════════════════════════════════
   BRANCHES TABLE
   ── CHANGED: dot colour now based on implementation_date
      green = Active (date set), red = Inactive (null)
════════════════════════════════════════════════════ */
function msaEndStatus(msaEnd?: string): { label: string; color: string } {
  if (!msaEnd) return { label: '—', color: '#b8aed8' };
  const now = new Date(); now.setHours(0, 0, 0, 0);
  const days = Math.ceil((new Date(msaEnd).getTime() - now.getTime()) / 86400000);
...
function BranchesTable({
  ...
  setBranchDetailModal,
}: BranchesTableProps) {
  const [hoveredRow,   setHoveredRow]   = useState<string | null>(null);
  const [branchSearch, setBranchSearch] = useState('');

  /* ── implementation_date map: branch_name → date string | null ── */
  const [implDates, setImplDates] = useState<Record<string, string | null>>({});
    ...
    fetch(`${API_BASE}/api/branches?company_id=${currentClient.id}`, {
      ...
      .then(d => {
        ...
        (d.branches ?? []).forEach((b: any) => {
          map[b.branch_name] = b.implementation_date ?? null;
        });
        setImplDates(map);
      })
      .catch(() => {});
  ...
  const sharedLicense = !isAloha ? resolveSharedLicense(currentClient) : '';

  const filteredBranches = branchSearch.trim()
    ? branches.filter(b => b.toLowerCase().includes(branchSearch.trim().toLowerCase()))
    : branches;
    ...
    <div style={{ background: '#fff', border: '1px solid rgba(124,58,237,0.1)', borderRadius: 12, display: 'flex', flexDirection: 'column', overflow: 'hidden', height: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderBottom: '1px solid rgba(124,58,237,0.1)', flexShrink: 0, gap: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
          ...
          <span style={{ fontSize: 9.5, fontWeight: 700, background: '#ede9fe', color: '#7c3aed', padding: '2px 8px', borderRadius: 10, flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
          ...
          <div style={{ width: 155, display: 'flex', alignItems: 'center', gap: 6, background: '#f2f0fb', border: '1px solid rgba(124,58,237,0.12)', borderRadius: 8, padding: '5px 10px' }}>
            <svg viewBox="0 0 16 16" fill="none" stroke="#8e7ec0" strokeWidth="1.6" width="11" height="11" style={{ flexShrink: 0 }}>
              <circle cx="6.5" cy="6.5" r="4.5"/><path d="M11 11l3 3"/>
            </svg>
            <input
              style={{ background: 'none', border: 'none', outline: 'none', fontSize: 11.5, color: '#18103a', width: '100%', minWidth: 0, fontFamily: "'DM Sans',sans-serif" }}
          ...
          <button
            ...
            style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 10px', borderRadius: 8, fontSize: 10.5, fontWeight: 600, color: '#4a3870', background: '#f2f0fb', border: '1px solid rgba(124,58,237,0.1)', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", transition: 'all 0.15s', whiteSpace: 'nowrap' }}
      ...
      <div style={{ flex: 1, overflowY: 'auto', scrollbarWidth: 'thin', scrollbarColor: 'rgba(124,58,237,0.15) transparent' } as React.CSSProperties}>
        {branchesLoading ? (
          <div style={{ padding: '10px 14px', display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[90, 75, 85].map((w, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 0', borderBottom: '1px solid rgba(124,58,237,0.06)' }}>
                <div style={{ width: w, height: 12, borderRadius: 5, background: 'linear-gradient(90deg,#ede9fe 25%,#f5f3ff 50%,#ede9fe 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s infinite' }} />
                <div style={{ width: 50, height: 10, borderRadius: 5, background: 'linear-gradient(90deg,#ede9fe 25%,#f5f3ff 50%,#ede9fe 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s infinite' }} />
              </div>
            ))}
          </div>
        ) : branches.length === 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '32px 20px', textAlign: 'center' }}>
            <div style={{ width: 40, height: 40, borderRadius: 10, background: '#f2f0fb', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <svg width="20" height="20" viewBox="0 0 16 16" fill="none" stroke="#b8aed8" strokeWidth="1.4"><path d="M8 1.5C5.8 1.5 4 3.3 4 5.5c0 3.3 4 8 4 8s4-4.7 4-8c0-2.2-1.8-4-4-4z"/><circle cx="8" cy="5.5" r="1.5"/></svg>
            </div>
            <span style={{ fontSize: 12, fontWeight: 600, color: '#4a3870' }}>No branches yet</span>
            <span style={{ fontSize: 11, color: '#b8aed8' }}>Click <strong style={{ color: '#7c3aed' }}>+ Add Branch</strong> to get started.</span>
          </div>
        ) : filteredBranches.length === 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '28px 20px', textAlign: 'center' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr>
                {['Branch', isAloha ? 'License' : 'Shared License', 'POS Devices', 'MSA End', ''].map((h, i) => (
                  <th key={i} style={{ position: 'sticky', top: 0, zIndex: 1, background: '#f2f0fb', padding: '7px 14px', fontSize: 9.5, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.09em', color: '#8e7ec0', textAlign: i === 2 ? 'center' : i === 4 ? 'right' : 'left', borderBottom: '1px solid rgba(124,58,237,0.1)', whiteSpace: 'nowrap' }}>
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {filteredBranches.map((branch: string) => {
                const branchPosDevices = posDevices.filter(d => d.branch === branch);
                const posCount         = branchPosDevices.length;
                const license = isAloha
                  ? (currentClient.branchLicenses?.[branch]?.trim() || '—')
                  : (sharedLicense || '—');
                const msaEndRaw = currentClient.branchMsaDates?.[branch]?.msaEnd || branchPosDevices.find(p => p.msaEnd)?.msaEnd;
                const { label: msaLabel, color: msaColor } = msaEndStatus(msaEndRaw);
                const isHovered = hoveredRow === branch;

                /* ── Active/Inactive dot colour based on implementation_date ── */
                const isActive  = implDates.hasOwnProperty(branch) ? implDates[branch] != null : null;
                const dotColor  = isActive === null
                  ? '#b8aed8'       /* not yet loaded — neutral grey */
                  : isActive
                  ? '#16a34a'       /* Active   — green */
                  : '#dc2626';      /* Inactive — red   */

                return (
                  <tr key={branch} style={{ borderTop: '1px solid rgba(124,58,237,0.07)', transition: 'background 0.12s' }}
                    ...
                    <td style={{ padding: '10px 14px', background: isHovered ? '#faf9ff' : 'transparent' }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 7 }}>
                        <span style={{ width: 7, height: 7, borderRadius: '50%', flexShrink: 0, marginTop: 4, background: dotColor, transition: 'background 0.2s' }} />
                        <span style={{ fontWeight: 600, color: '#18103a', lineHeight: 1.45, wordBreak: 'break-word', overflowWrap: 'break-word', whiteSpace: 'normal' }}>
                          {branch}
                        </span>
                      </div>
                    </td>
                    ...
                    <td style={{ padding: '10px 14px', background: isHovered ? '#faf9ff' : 'transparent' }}>
                      {license !== '—' ? (
                        <span style={{ fontSize: 10.5, fontWeight: 600, background: isAloha ? 'rgba(217,119,6,0.08)' : 'rgba(124,58,237,0.07)', color: isAloha ? '#92400e' : '#5b21b6', border: `1px solid ${isAloha ? 'rgba(217,119,6,0.2)' : 'rgba(124,58,237,0.15)'}`, borderRadius: 5, padding: '2px 7px', whiteSpace: 'nowrap', display: 'inline-block', maxWidth: 130, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                          {license}
                        </span>
                      ) : (
                    ...
                    {/* POS count */}
                    <td style={{ padding: '10px 14px', textAlign: 'center', background: isHovered ? '#faf9ff' : 'transparent' }}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minWidth: 24, fontSize: 11, fontWeight: 700, background: posCount > 0 ? '#ede9fe' : '#f2f0fb', color: posCount > 0 ? '#5b21b6' : '#b8aed8', borderRadius: 20, padding: '2px 9px' }}>
                        {posCount}
                      </span>
                    </td>
                    ...
                    <td style={{ padding: '10px 14px', background: isHovered ? '#faf9ff' : 'transparent' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                        {msaEndRaw && <span style={{ width: 6, height: 6, borderRadius: '50%', background: msaColor, flexShrink: 0 }} />}
                        <span style={{ fontSize: 11, fontWeight: msaEndRaw ? 600 : 400, color: msaColor, whiteSpace: 'nowrap' }}>{msaLabel}</span>
                      </div>
                    </td>

                    {/* View button */}
                    <td style={{ padding: '10px 14px', textAlign: 'right', background: isHovered ? '#faf9ff' : 'transparent', whiteSpace: 'nowrap' }}>
                      <button
                        onClick={() => setBranchDetailModal({ branch, client: currentClient })}
                        style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '5px 11px', borderRadius: 7, fontSize: 10.5, fontWeight: 600, color: isHovered ? '#7c3aed' : '#4a3870', background: isHovered ? '#ede9fe' : '#f2f0fb', border: `1px solid ${isHovered ? 'rgba(124,58,237,0.3)' : 'rgba(124,58,237,0.12)'}`, cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", transition: 'all 0.14s', whiteSpace: 'nowrap' }}
                      >
                        View
...
/* ════════════════════════════════════════════════════
   LICENSE & ACTIVATION DISPLAY ROW
════════════════════════════════════════════════════ */
function LicenseActivationRow({ client, onEdit }: { client: Client; onEdit: () => void }) {
  const krunch     = client.krunchNum?.trim() || null;
  const activation = client.saStart?.trim()   || null;
  if (!krunch && !activation) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, padding: '5px 0', borderBottom: '1px solid rgba(124,58,237,0.05)', minHeight: 26 }}>
        ...
        <button onClick={onEdit} style={{ display: 'inline-flex', alignItems: 'center', padding: '3px 10px', borderRadius: 5, fontSize: 9.5, fontWeight: 600, color: '#4a3870', background: '#f2f0fb', border: '1px solid rgba(124,58,237,0.1)', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif" }}>+ Add</button>
      </div>
    );
  }
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      {krunch && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, padding: '5px 0', borderBottom: '1px solid rgba(124,58,237,0.05)', minHeight: 26 }}>
          <span style={{ fontSize: 10.5, color: '#8e7ec0', fontWeight: 500, whiteSpace: 'nowrap', flexShrink: 0, minWidth: 100 }}>Krunch No.</span>
          <span style={{ fontSize: 11, fontWeight: 700, background: 'rgba(13,148,136,0.08)', color: '#0d7873', border: '1px solid rgba(13,148,136,0.2)', borderRadius: 5, padding: '2px 8px', fontFamily: "'DM Mono', monospace, 'DM Sans', sans-serif", letterSpacing: '0.03em' }}>{krunch}</span>
        </div>
      )}
      {activation && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, padding: '5px 0', borderBottom: '1px solid rgba(124,58,237,0.05)', minHeight: 26 }}>
          <span style={{ fontSize: 10.5, color: '#8e7ec0', fontWeight: 500, whiteSpace: 'nowrap', flexShrink: 0, minWidth: 100 }}>Activation Code</span>
          <span style={{ fontSize: 11, fontWeight: 700, background: 'rgba(124,58,237,0.08)', color: '#5b21b6', border: '1px solid rgba(124,58,237,0.18)', borderRadius: 5, padding: '2px 8px', fontFamily: "'DM Mono', monospace, 'DM Sans', sans-serif", letterSpacing: '0.03em' }}>{activation}</span>
        </div>
      )}
    </div>
  );
...
export function OverviewPanel({
  ...
  branchesLoading, setOvPanel, setShowAddBranchModal, setMsaModalOpen,
  setBranchDetailModal, openEditInfoModal,
}: OverviewPanelProps) {
  return (
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '5fr 5fr', gap: 12, overflow: 'hidden', minHeight: 0 }}>
        ...
        <div style={{ background: '#fff', border: '1px solid rgba(124,58,237,0.1)', borderRadius: 12, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', borderBottom: '1px solid rgba(124,58,237,0.1)', flexShrink: 0 }}>
            ...
            <button onClick={() => openEditInfoModal(currentClient)} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 10px', borderRadius: 8, fontSize: 10.5, fontWeight: 600, color: '#4a3870', background: '#f2f0fb', border: '1px solid rgba(124,58,237,0.1)', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", transition: 'all 0.15s' }}
              onMouseEnter={e => { const el = e.currentTarget; el.style.background = '#fff'; el.style.borderColor = 'rgba(124,58,237,0.22)'; el.style.color = '#18103a'; }}
              onMouseLeave={e => { const el = e.currentTarget; el.style.background = '#f2f0fb'; el.style.borderColor = 'rgba(124,58,237,0.1)'; el.style.color = '#4a3870'; }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '10px 14px 14px', display: 'flex', flexDirection: 'column', gap: 0, scrollbarWidth: 'thin', scrollbarColor: 'rgba(124,58,237,0.15) transparent' } as React.CSSProperties}>
            ...
            {currentClient.altContact ? (
              [{ key: 'Contact Person', val: currentClient.altContact }, { key: 'Email', val: currentClient.altEmail || '—' }, { key: 'Phone', val: currentClient.altPhone || '—' }].map(item => <InfoRow key={`alt1-${item.key}`} label={item.key} value={item.val} />)
            ) : (
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, padding: '5px 0', borderBottom: '1px solid rgba(124,58,237,0.05)', minHeight: 26 }}>
            ...
            {currentClient.altContact2 ? (
              [{ key: 'Contact Person', val: currentClient.altContact2 }, { key: 'Email', val: currentClient.altEmail2 || '—' }, { key: 'Phone', val: currentClient.altPhone2 || '—' }].map(item => <InfoRow key={`alt2-${item.key}`} label={item.key} value={item.val} />)
            ) : (
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, padding: '5px 0', borderBottom: '1px solid rgba(124,58,237,0.05)', minHeight: 26 }}>
            ...
            <LicenseActivationRow client={currentClient} onEdit={() => openEditInfoModal(currentClient)} />
        ...
        <BranchesTable currentClient={currentClient} posDevices={posDevices} branchesLoading={branchesLoading} setShowAddBranchModal={setShowAddBranchModal} setBranchDetailModal={setBranchDetailModal} />
      </div>
    </div>
  );
}
...
export function TicketsPanel({ currentClient, clientTicketCounts, ticketStatusFilter, setTicketStatusFilter, ticketPeriod, setTicketPeriod, showToast }: TicketsPanelProps) {
    ...
    <div style={{ width: '100%', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 10, overflow: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 3, background: '#f2f0fb', border: '1px solid rgba(124,58,237,0.1)', borderRadius: 9, padding: 3, flexShrink: 0 }}>
          {(['7D', '30D', '90D'] as TicketPeriod[]).map(p => (
            <div key={p} onClick={() => { setTicketPeriod(p); setShowCustomPicker(false); }} style={{ padding: '5px 11px', borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: 'pointer', transition: 'all 0.14s', userSelect: 'none', background: ticketPeriod === p ? '#fff' : 'transparent', color: ticketPeriod === p ? '#7c3aed' : '#8e7ec0', boxShadow: ticketPeriod === p ? '0 1px 4px rgba(0,0,0,0.08)' : 'none' }}>{p}</div>
          ))}
          <div style={{ position: 'relative' }} ref={customPickerRef}>
            <div onClick={() => setShowCustomPicker(p => !p)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 11px', borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: 'pointer', transition: 'all 0.14s', userSelect: 'none', background: ticketPeriod === 'custom' ? '#fff' : 'transparent', color: ticketPeriod === 'custom' ? '#7c3aed' : '#8e7ec0', boxShadow: ticketPeriod === 'custom' ? '0 1px 4px rgba(0,0,0,0.08)' : 'none' }}>
              Custom
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {[['From', customFrom, setCustomFrom], ['To', customTo, setCustomTo]].map(([label, val, setter]) => (
                    <div key={label as string} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <input type="date" value={val as string} onChange={e => (setter as React.Dispatch<React.SetStateAction<string>>)(e.target.value)} style={{ flex: 1, padding: '5px 8px', borderRadius: 7, border: '1px solid rgba(124,58,237,0.18)', fontSize: 11, fontFamily: "'DM Sans',sans-serif", color: C.t1, outline: 'none', background: '#faf9ff' }} />
                    </div>
                  ))}
                </div>
                <button onClick={() => { if (customFrom && customTo) { setTicketPeriod('custom'); setShowCustomPicker(false); } }} style={{ marginTop: 12, width: '100%', padding: '7px 0', borderRadius: 8, background: 'linear-gradient(135deg,#7c3aed,#5b21b6)', color: '#fff', fontSize: 11.5, fontWeight: 700, border: 'none', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", opacity: customFrom && customTo ? 1 : 0.45 }}>
                  Apply Range
                </button>
              </div>
            )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.15fr 1fr', gap: 10, flex: 1, minHeight: 0, overflow: 'hidden' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, overflow: 'hidden', minHeight: 0 }}>
          ...
          <div style={{ flex: 1, overflow: 'hidden', minHeight: 0 }}><ClientCategoryChart clientId={currentClient.id} /></div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, overflow: 'hidden', minHeight: 0 }}>
...
function accessLevelBadgeStyle(level: string): React.CSSProperties {
  switch (level) {
    case 'super_admin':  return { background: '#fce7f3', color: '#9d174d' };
    case 'system_admin': return { background: '#e0f2fe', color: '#0c4a6e' };
    case 'manager':      return { background: '#fce7f3', color: '#be185d' };
    default:             return { background: '#f4f4f8', color: '#4a3870' };
  }
}
...
export function UsersPanel({ currentClient, dbUsers, dbUsersLoading, userSearch, setUserSearch, userRoleFilters, userStatusFilters, userPositionFilters, setUserRoleFilters, setUserStatusFilters, setUserPositionFilters, setAddUserModalOpen, showToast, onRefreshUsers }: UsersPanelProps) {
  ...
  const filteredUsers = dbUsers.filter(u => {
    const roleMatch     = userRoleFilters.size === 0     || userRoleFilters.has(accessLevelLabel(u.access_level));
    const statusMatch   = userStatusFilters.size === 0   || userStatusFilters.has(u.status === 'active' ? 'Active' : 'Inactive');
    const positionMatch = userPositionFilters.size === 0 || userPositionFilters.has(u.position_title || '');
    const searchMatch   = !q || u.full_name.toLowerCase().includes(q) || u.email.toLowerCase().includes(q) || (u.position_title || '').toLowerCase().includes(q);
    return roleMatch && statusMatch && positionMatch && searchMatch;
  });

  const uniquePositions    = Array.from(new Set(dbUsers.map(u => u.position_title || '').filter(Boolean))).sort();
  ...
  const openFilterPanel = () => { setPendingRoles(new Set(userRoleFilters)); setPendingStatuses(new Set(userStatusFilters)); setPendingPositions(new Set(userPositionFilters)); setShowUserFilterPanel(true); };
  const applyFilters = () => { setUserRoleFilters(new Set(pendingRoles)); setUserStatusFilters(new Set(pendingStatuses)); setUserPositionFilters(new Set(pendingPositions)); setShowUserFilterPanel(false); };
  ...
  return (
    <div style={{ width: '100%', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 12, overflow: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        ...
        <button onClick={onRefreshUsers} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 8, fontSize: 11.5, fontWeight: 600, color: '#4a3870', cursor: 'pointer', border: '1px solid rgba(124,58,237,0.15)', background: '#f2f0fb', fontFamily: "'DM Sans',sans-serif" }}>
          <svg viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.7" width="12" height="12"><path d="M12 7A5 5 0 1 1 7 2"/><path d="M7 2l3 0 0 3"/></svg>
          Refresh
        </button>
      </div>

      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#f2f0fb', border: '1px solid rgba(124,58,237,0.1)', borderRadius: 9, padding: '6px 12px', flex: 1, maxWidth: 300 }}>
        ...
        <div ref={filterPanelRef} style={{ position: 'relative' }}>
          <button onClick={() => showUserFilterPanel ? setShowUserFilterPanel(false) : openFilterPanel()} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 9, cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", fontSize: 12, fontWeight: 600, transition: 'all 0.16s', background: showUserFilterPanel ? '#7c3aed' : '#f2f0fb', color: showUserFilterPanel ? '#fff' : '#4a3870', border: showUserFilterPanel ? 'none' : '1px solid rgba(124,58,237,0.16)', boxShadow: showUserFilterPanel ? '0 2px 10px rgba(124,58,237,0.3)' : 'none' }}>
            ...
            <div style={{ position: 'absolute', top: 'calc(100% + 8px)', left: 0, zIndex: 200, minWidth: 520, background: '#fff', borderRadius: 14, border: '1px solid rgba(124,58,237,0.13)', boxShadow: '0 8px 32px rgba(124,58,237,0.14)', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1.8fr', gap: 0 }}>
                <div style={{ padding: '14px 18px 10px', borderRight: '1px solid rgba(124,58,237,0.08)' }}>
                  <span style={{ fontSize: 9.5, fontWeight: 800, color: '#8e7ec0', letterSpacing: '0.12em', textTransform: 'uppercase', display: 'block', marginBottom: 10 }}>Access Level</span>
                  {['Manager', 'User', 'System Admin', 'Super Admin'].map(r => (
                ...
                <div style={{ padding: '14px 18px 10px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 0, maxHeight: 160, overflowY: 'auto', scrollbarWidth: 'thin', scrollbarColor: 'rgba(124,58,237,0.15) transparent' } as React.CSSProperties}>
                    {uniquePositions.length === 0 ? <span style={{ fontSize: 11, color: '#b8aed8', fontStyle: 'italic' }}>No positions available</span> : uniquePositions.map(position => (
        ...
        <div style={{ flex: 1 }} />
        <span style={{ fontSize: 11, color: '#8e7ec0' }}>{dbUsersLoading ? 'Loading…' : `${filteredUsers.length} user${filteredUsers.length !== 1 ? 's' : ''}`}</span>
      </div>
      ...
      <div style={{ background: '#fff', border: '1px solid rgba(124,58,237,0.1)', borderRadius: 12, flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {dbUsersLoading ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, color: '#8e7ec0', fontSize: 13 }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '40px 0', color: '#b8aed8' }}>
            ...
            <span style={{ fontSize: 11, color: '#b8aed8' }}>{userSearch || totalActiveFilters > 0 ? 'Try adjusting your search or filters' : `No users assigned to ${currentClient.name} yet.`}</span>
          </div>
        ) : (
          <div style={{ flex: 1, overflowY: 'auto', scrollbarWidth: 'thin', scrollbarColor: 'rgba(124,58,237,0.15) transparent' } as React.CSSProperties}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead>
                <tr>
                  {['Full Name', 'Email', 'Access', 'Position', 'Status'].map(h => (
                    <th key={h} style={{ position: 'sticky', top: 0, background: '#f2f0fb', padding: '8px 14px', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#8e7ec0', textAlign: 'left', borderBottom: '1px solid rgba(124,58,237,0.1)', whiteSpace: 'nowrap', zIndex: 1 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filteredUsers.map((u: DBUser) => {
                  const [g1, g2]    = getAvatarGradient(u.full_name);
                  const accessLabel = accessLevelLabel(u.access_level);
                  const accessStyle = accessLevelBadgeStyle(u.access_level);
                  ...
                  return (
                    <tr key={u.id}
                      ...
                      <td style={{ padding: '9px 14px', borderBottom: '1px solid rgba(124,58,237,0.07)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                          {u.profile_photo ? (
                            <img src={u.profile_photo} alt={u.full_name} style={{ width: 32, height: 32, borderRadius: 8, objectFit: 'cover', flexShrink: 0, border: '1.5px solid rgba(124,58,237,0.12)' }} onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                          ) : (
                            <div style={{ width: 32, height: 32, borderRadius: 8, flexShrink: 0, background: `linear-gradient(135deg,${g1},${g2})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, color: '#fff', letterSpacing: '0.03em', boxShadow: `0 2px 6px ${g1}55`, userSelect: 'none' }}>
                              {getInitials(u.full_name)}
                            </div>
                          )}
                          <span style={{ fontWeight: 600, color: '#18103a' }}>{u.full_name}</span>
                        </div>
                      </td>
                      <td style={{ padding: '9px 14px', borderBottom: '1px solid rgba(124,58,237,0.07)', color: '#4a3870', fontSize: 11 }}>{u.email}</td>
                      <td style={{ padding: '9px 14px', borderBottom: '1px solid rgba(124,58,237,0.07)' }}>
                        <span style={{ display: 'inline-flex', alignItems: 'center', fontSize: 9.5, fontWeight: 700, letterSpacing: '0.04em', padding: '2px 8px', borderRadius: 20, ...accessStyle }}>{accessLabel}</span>
                      </td>
                      <td style={{ padding: '9px 14px', borderBottom: '1px solid rgba(124,58,237,0.07)', fontSize: 11, color: '#8e7ec0' }}>{u.position_title || '—'}</td>
                      <td style={{ padding: '9px 14px', borderBottom: '1px solid rgba(124,58,237,0.07)' }}>
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 9.5, fontWeight: 700, letterSpacing: '0.04em', padding: '2px 8px', borderRadius: 20, background: isActive ? '#dcfce7' : '#fee2e2', color: isActive ? '#14532d' : '#7f1d1d' }}>
                          <span style={{ width: 5, height: 5, borderRadius: '50%', flexShrink: 0, background: isActive ? '#22c55e' : '#ef4444' }} />
