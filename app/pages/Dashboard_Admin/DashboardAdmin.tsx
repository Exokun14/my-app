...
interface DashboardAdminProps {
  ...
  onLogout?: () => void;
  user?: any;  // passed from page.tsx — used to skip redundant /api/user fetch
}
...
export default function DashboardAdmin({ onClientSelect, onLogout, user }: DashboardAdminProps) {

  /* ── Real user data — use prop if available, else fall back to API ── */
  const { headerUser } = useAuthUser(user);
  ...
  useEffect(() => {
    console.log('🏢 [DashboardAdmin] useEffect fired — calling fetchCompanies');
    let cancelled = false;
    setClientsLoading(true);
    setClientsError(null);
    fetchCompanies().then(({ clients: loaded, error }) => {
      console.log('🏢 [DashboardAdmin] fetchCompanies resolved — error:', error, '| count:', loaded.length);
      if (cancelled) return;
      if (error) { setClientsError(error); }
      else        { setClients(loaded); }
                  ...
                  <div className="flex-1 overflow-y-auto min-h-0">
                    {clientsLoading ? (
                      <div className="flex flex-col items-center justify-center gap-3 p-10 text-center h-full">
                        ...
                        <div style={{ width: 32, height: 32, borderRadius: '50%', border: '3px solid rgba(124,58,237,0.15)', borderTopColor: '#7c3aed', animation: '_spin 0.75s linear infinite' }} />
                        <div style={{ fontSize: 12, color: '#8e7ec0', fontWeight: 500 }}>Loading companies…</div>
                      </div>
                    ) : clientsError ? (
                      <div className="flex flex-col items-center justify-center gap-2 p-10 text-center h-full">
                        <div style={{ fontSize: 28, opacity: 0.5 }}>⚠️</div>
                        <div style={{ fontSize: 13, fontWeight: 600, color: '#4a3870' }}>Could not load companies</div>
                        <div style={{ fontSize: 11, color: '#8e7ec0' }}>{clientsError}</div>
                        <button
                          onClick={() => {
                            setClientsLoading(true);
                            setClientsError(null);
                          ...
                          style={{ marginTop: 6, padding: '6px 18px', borderRadius: 8, background: '#7c3aed', color: '#fff', border: 'none', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}
                        >
                          Retry
                        </button>
                      </div>
                      ...
                      <>
                        <div className="grid grid-cols-3 gap-[10px] content-start pb-3">
                          {filtered.map((c, i) => (
                            <ClientCard key={c.id} client={c} index={i} onClick={() => onClientSelect?.(c)} />
                          ))}
                        </div>
                        {filtered.length === 0 && (
