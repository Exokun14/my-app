/* Admin sidebar component, used in DashboardAdmin and UserManagementMain. Contains navigation links. */
...
/* onNavigate(view): called instead of router.push when parent is in SPA mode.
   activePage: overrides URL-based active highlight (e.g. "learning").
   onLogout: wired to the sign-out button in the Header dropdown via AppShell.  */
...
const NAV_SECTIONS: NavSection[] = [
  {
    ...
    items: [
      ...
      { label: "Learning Center", page: "learning",  href: ROUTES.learning,  icon: <Icons.Learning /> },
    ],
  },
  {
    label: "Insights",
    items: [
      ...
      { label: "Users",     page: "users",     href: ROUTES.users,     icon: <Icons.Users /> },
    ],
  },
];

function checkActive(item: NavItem, pathname: string, activePage?: string): boolean {
  // If parent passes an explicit activePage, use it — bypasses URL matching
...
/* ─── Sidebar ─────────────────────────────────────────────────────────────── */
export default function Sidebar({ onNavigate, activePage, onLogout }: SidebarProps) {
  const pathname = usePathname();
  const router   = useRouter();
  const [collapsed, setCollapsed] = useState(false);

  // Inject styles once on the client to avoid SSR/client HTML mismatch
  useEffect(() => {
    const id = 'gx-sidebar-styles';
  ...
  const handleNav = (item: NavItem) => {
    // SPA mode: call onNavigate(page) so DashboardAdmin switches view via state
    if (onNavigate) {
      onNavigate(item.page);
      return;
    }
    // Fallback: normal Next.js routing (for pages that don't pass onNavigate)
    ...
    <>
      ...
      >
        <div className="gx-sidebar-inner" style={{ display: "flex", flexDirection: "column", height: "100%" }}>
          ...
          {/* ── Footer — Sign Out removed; logout is now in the Header dropdown ── */}
