...
const ROUTES = {
  /*
   * Dashboard always points to "/" — the root page.tsx state machine
   * handles all admin navigation. Direct Next.js routes for other admin
   * pages (Users, Analytics) redirect back to "/" if no session exists.
   */
  dashboard : "/",
  users     : "/pages/User_Management_Main",
  learning  : "/pages/Dashboard_Admin_Main/learning",
  analytics : "/pages/Analytics_Admin",
  login     : "/",          // logout → root (login gate will show if no session)
} as const;
...
/*
 * Treat both "/" and "/pages/Dashboard_Admin_Main" as the dashboard
 * so the active highlight works correctly in both cases.
 */
...
export default function Sidebar() {
  ...
  /*
   * FIX: Logout must clear the session key before navigating,
   * so the root page.tsx gate shows the login form instead of
   * the admin dashboard.
   */
  /*
   * FIX: Clear sessionStorage then use window.location.href instead of
   * router.push("/"). router.push does not remount the root component,
   * so the useState initializer that reads sessionStorage never re-runs
   * and the user stays logged in. A full page navigation forces a remount,
   * which re-evaluates the session (now empty) and shows the login screen.
   */
  const handleLogout = () => {
  ...
  /*
   * FIX: Navigation handler for sidebar items.
   * "Company DB" (href="/") always goes to root — the root page.tsx
   * reads sessionStorage and renders DashboardAdmin directly, so there
   * is no login flash and no state loss.
   */
