...
// ─── Stat chip with portal-style dropdown ─────────────────────────────────────
// Dropdown renders into document.body via portal so it's NEVER clipped by any
// scrolling ancestor's overflow.
function StatChip({ n, label, emoji, cls, delay, courses, onOpenCourse }: {
  ...
  // compute & track position while open
  useEffect(()=>{
    if(!open) return;
    const update = () => {
          ...
          {items.map(({c,i},idx)=>{
            ...
            return (
              <div key={idx} className="lv-dstat-row"
                ...
                <div className="lv-dstat-row-thumb"
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  {pct>0 && (
                    <div style={{display:"flex",alignItems:"center",gap:5,marginTop:3}}>
                      <div style={{flex:1,height:2,borderRadius:2,background:"rgba(108,61,214,.1)",overflow:"hidden"}}>
                        <div style={{height:"100%",width:`${pct}%`,background:"linear-gradient(90deg,var(--vio),var(--teal))",borderRadius:2}}/>
                      </div>
                      <span style={{fontSize:9,fontWeight:700,color:done?"#16a34a":"var(--vio)",flexShrink:0}}>
                        {done?"✓":pct+"%"}
                      </span>
                    </div>
                  )}
                </div>
                <svg width="8" height="8" viewBox="0 0 14 14" fill="none" stroke="var(--ink3)" strokeWidth="2" style={{flexShrink:0}}>
                  <path d="M5 2l4 5-4 5"/>
                </svg>
              </div>
            );
  ...
  return (
    <div ref={ref} className={`lv-dstat ${cls}`} style={{ animationDelay:delay }}>
      <div className="lv-dstat-inner" onClick={handleToggle}
        ...
        {items.length>0 && (
          <svg width="10" height="10" viewBox="0 0 14 14" fill="none" stroke="var(--ink3)" strokeWidth="2.2"
    ...
    <div className="lv-dstat sr" style={{ animationDelay:delay }}>
      <div className="lv-dstat-inner" style={{cursor:"default"}}>
        <div style={{position:"relative",display:"flex",alignItems:"center",justifyContent:"center"}}>
          <MiniRing pct={avg}/>
          <div style={{position:"absolute",fontSize:8,fontWeight:900,color:"#6c3dd6",lineHeight:1}}>{avg}%</div>
        </div>
        <div>
  ...
  return (
    <div className="lv-car2">
      <div className="lv-sh" style={{marginBottom:12}}>
        <div style={{width:3,height:13,borderRadius:2,background:"linear-gradient(#6c3dd6,#0d9488)",flexShrink:0}}/>
      ...
      <div className="lv-car2-card" onClick={()=>onOpenCourse(ri)}>
        ...
        <div className="lv-car2-thumb" style={{background:`linear-gradient(145deg,${g[0]},${g[1]})`}}>
          ...
          {/* emoji badge */}
          <div style={{position:"absolute",top:12,right:12,width:38,height:38,borderRadius:11,background:"rgba(255,255,255,.15)",backdropFilter:"blur(8px)",border:"1.5px solid rgba(255,255,255,.22)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>
            {ic}
          </div>
          {/* bottom text */}
          <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"10px 13px 9px"}}>
            <div style={{fontSize:8,fontWeight:800,textTransform:"uppercase",letterSpacing:".08em",color:"rgba(255,255,255,.5)",marginBottom:3,display:"flex",alignItems:"center",gap:4}}>
            ...
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:14,fontWeight:700,fontStyle:"italic",color:"#fff",lineHeight:1.25,textShadow:"0 2px 8px rgba(0,0,0,.3)",display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>
              {c.title}
            </div>
          </div>
        </div>
          ...
          {enr&&pct>0 && (
            <div style={{marginBottom:9}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
                <span style={{fontSize:9,fontWeight:700,color:"var(--ink3)"}}>Progress</span>
                <span style={{fontSize:9,fontWeight:800,color:"var(--vio)"}}>{pct}%</span>
              </div>
              <div style={{height:3.5,borderRadius:4,background:"rgba(108,61,214,.08)",overflow:"hidden"}}>
                <div style={{height:"100%",width:`${pct}%`,background:"linear-gradient(90deg,var(--vio),var(--teal))",borderRadius:4}}/>
              </div>
            </div>
          )}
          {c.desc && (
            <div style={{fontSize:10.5,color:"var(--ink3)",lineHeight:1.55,marginBottom:9,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>
              {c.desc}
            </div>
          )}
          <div style={{display:"flex",gap:5,marginBottom:10,flexWrap:"wrap"}}>
            {c.time && (
              <span style={{display:"flex",alignItems:"center",gap:3,fontSize:9.5,fontWeight:600,color:"var(--ink3)",padding:"3px 8px",borderRadius:20,background:"var(--surf2)",border:"1px solid var(--bd)"}}>
            ...
            {enr && (
              <span style={{fontSize:9.5,fontWeight:700,padding:"3px 8px",borderRadius:20,background:done?"rgba(22,163,74,.08)":"rgba(108,61,214,.08)",color:done?"#16a34a":"var(--vio)",border:`1px solid ${done?"rgba(22,163,74,.14)":"rgba(108,61,214,.14)"}`}}>
            ...
            <div key={idx} className="lv-strip-card" style={{animationDelay:`${idx*.06}s`}} onClick={()=>onOpenCourse(i)}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div className="lv-strip-bar-wrap">
                  {done
                    ? <span style={{fontSize:9.5,fontWeight:700,color:"#16a34a"}}>✓ Completed</span>
                    : <>
...
.lv-hero {
  ...
  box-shadow:0 8px 32px rgba(108,61,214,.22), 0 2px 8px rgba(0,0,0,.12);
...
.lv-hero-badge {
  ...
  font-size:8.5px; font-weight:800; color:rgba(255,255,255,.7);
...
.lv-hero-summary {
  font-size:12px; color:rgba(255,255,255,.6); line-height:1.65; margin:0;
}
...
.lv-hero-pill-n { display:block; font-size:17px; font-weight:900; color:#fff; line-height:1; letter-spacing:-.03em; }
.lv-hero-pill-l { display:block; font-size:8.5px; font-weight:700; color:rgba(255,255,255,.5); margin-top:2px; text-transform:uppercase; letter-spacing:.05em; }
...
.lv-hero-cta {
  ...
  transition:all .16s; backdrop-filter:blur(4px);
}
.lv-hero-cta:hover { background:rgba(255,255,255,.17); border-color:rgba(255,255,255,.28); transform:translateX(-2px); }
.lv-hero-cta-ico  { font-size:16px; flex-shrink:0; }
.lv-hero-cta-lbl  { font-size:8.5px; font-weight:800; color:rgba(255,255,255,.5); letter-spacing:.07em; text-transform:uppercase; margin-bottom:2px; }
.lv-hero-cta-name { font-size:11.5px; font-weight:700; color:#fff; }
.lv-hero-cta-arr  { margin-left:auto; color:rgba(255,255,255,.5); font-size:15px; transition:transform .16s; }
...
.lv-hero-browse:hover { background:rgba(255,255,255,.17); }
...
.lv-dstat-ico  { font-size:15px; flex-shrink:0; }
.lv-dstat-num  { font-size:20px; font-weight:900; color:var(--ink); line-height:1; letter-spacing:-.03em; }
.lv-dstat-lbl  { font-size:9px; font-weight:700; color:var(--ink3); margin-top:1px; text-transform:uppercase; letter-spacing:.04em; }

/* dropdown — fixed position via portal, never clipped */
.lv-dstat-drop {
  border-radius:12px;
  border:1.5px solid #e5e7eb;
  box-shadow:0 12px 36px rgba(108,61,214,.18), 0 4px 12px rgba(0,0,0,.14);
  animation:lvFadeUp .18s cubic-bezier(.16,1,.3,1) both;
  max-height:230px; overflow-y:auto;
}
.lv-dstat-drop::-webkit-scrollbar { width:3px; }
.lv-dstat-drop::-webkit-scrollbar-thumb { background:#a78bfa; border-radius:3px; }
.lv-dstat-drop-head {
...
.lv-dstat-row:hover { background:#f5f3ff; }
.lv-dstat-row-thumb {
...
.lv-strip-card:hover { border-color:rgba(108,61,214,.22); transform:translateX(3px); box-shadow:0 3px 10px rgba(108,61,214,.08); }
.lv-strip-thumb {
  width:34px; height:34px; border-radius:9px; flex-shrink:0;
  display:flex; align-items:center; justify-content:center;
  position:relative; overflow:hidden;
}
.lv-strip-name { font-size:11px; font-weight:800; color:var(--ink); margin-bottom:4px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.lv-strip-bar-wrap { display:flex; align-items:center; gap:5px; }
.lv-strip-bar  { flex:1; height:2.5px; border-radius:3px; background:rgba(108,61,214,.1); overflow:hidden; }
...
.lv-strip-pct  { font-size:9px; font-weight:700; color:var(--vio); flex-shrink:0; }
.lv-strip-btn {
  flex-shrink:0; width:24px; height:24px; border-radius:7px; border:none; cursor:pointer;
  background:linear-gradient(135deg,var(--vio),#4f1eb8); color:#fff;
  display:flex; align-items:center; justify-content:center;
  transition:all .15s; box-shadow:0 2px 6px rgba(108,61,214,.24);
}
.lv-strip-btn:hover { transform:scale(1.1); }
...
.lv-car2-card {
  ...
  box-shadow:0 4px 18px rgba(108,61,214,.09), 0 1px 4px rgba(0,0,0,.05);
  display:flex; flex-direction:column; cursor:pointer;
  transition:box-shadow .18s,transform .18s; background:var(--surf);
}
.lv-car2-card:hover { transform:translateY(-2px); box-shadow:0 8px 26px rgba(108,61,214,.14); }
.lv-car2-thumb {
...
.lv-car2-body {
  flex:1; padding:13px 14px 11px;
  display:flex; flex-direction:column; background:var(--surf);
}
.lv-car2-cta {
  width:100%; padding:7px 12px; border-radius:9px; border:none; cursor:pointer;
  font-size:11.5px; font-weight:800; font-family:'Sora',sans-serif;
  display:flex; align-items:center; justify-content:center; gap:5px;
  transition:all .15s; margin-top:auto;
}
.lv-car2-cta:hover { transform:translateY(-1px); filter:brightness(1.06); }
.lv-car2-cta.new  { background:rgba(108,61,214,.08); color:var(--vio); border:1.5px solid rgba(108,61,214,.13); }
.lv-car2-cta.new:hover { background:rgba(108,61,214,.13); }
.lv-car2-cta.enr  { background:linear-gradient(135deg,var(--vio),#4f1eb8); color:#fff; box-shadow:0 3px 10px rgba(108,61,214,.32); }
.lv-car2-cta.done { background:linear-gradient(135deg,#065f46,#0d9488); color:#fff; }
.lv-car2-nav {
  display:flex; align-items:center; justify-content:space-between;
  padding:8px 0 0; flex-shrink:0;
}
.lv-car2-dots { display:flex; gap:4px; align-items:center; }
.lv-car2-dot  { width:5px; height:5px; border-radius:3px; cursor:pointer; background:rgba(108,61,214,.15); transition:all .22s cubic-bezier(.16,1,.3,1); }
.lv-car2-dot.on { width:18px; background:linear-gradient(90deg,var(--vio),var(--teal)); box-shadow:0 1px 5px rgba(108,61,214,.28); }
.lv-car2-arr {
...
.lv-car2-arr:hover { border-color:rgba(108,61,214,.28); color:var(--vio); background:rgba(108,61,214,.05); }
`;
