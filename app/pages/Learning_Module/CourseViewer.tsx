...
.cv-accordion-answer {
  ...
  font-size:13px; color:var(--t2,#4a3870); line-height:1.65;
...
export default function CourseViewer({ course, onClose, onProgress, toast }: CourseViewerProps) {
  ...
  const areAllPreviousChaptersComplete = () => {
    for (let m = 0; m <= selMod; m++) {
      ...
      for (let c = 0; c <= maxCh; c++) {
        if (!mod.chapters[c].done) return false;
      }
    }
    return true;
  };
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (progress >= 95 && currentChapter && !currentChapter.done) {
        ...
        if (hasVideo) {
          const allVideosWatched = blocks.filter(b => b.type === 'media').every(b => videoWatched[b.id]);
        ...
        const completedChapters = modules.reduce((sum, m) => sum + m.chapters.filter(c => c.done).length, 0);
        const newProgressPercent = totalChapters > 0 ? Math.round((completedChapters / totalChapters) * 100) : 0;
        const timeSpent = calcTimeSpent();

        setSaving(true); setSavingMsg("Saving progress...");
        onProgress(newProgressPercent, timeSpent);
  ...
  const markComplete = () => {
    ...
    if (currentChapter.type === 'quiz') {
      ...
      if (!allCorrect) { toast("⚠️ Please answer all questions correctly before continuing."); return; }
      if (!currentChapter.done) toast("✅ Quiz passed! Great work!");
      currentChapter.done = true;
      const totalChapters = modules.reduce((sum, m) => sum + m.chapters.length, 0);
      const completedChapters = modules.reduce((sum, m) => sum + m.chapters.filter(c => c.done).length, 0);
      const newProgressPercent = totalChapters > 0 ? Math.round((completedChapters / totalChapters) * 100) : 0;
      const timeSpent = calcTimeSpent();
      setSaving(true); setSavingMsg("Saving quiz result...");
      onProgress(newProgressPercent, timeSpent);
      setTimeout(() => setSaving(false), 1200);
    }

    if (currentChapter.type === 'assessment') return;

    if (currentChapter.type === 'lesson') currentChapter.done = true;
  ...
  const submitAssessment = () => {
    ...
    setAssessmentScores(prev => ({ ...prev, [chapterKey]: [...(prev[chapterKey] || []), scorePercent] }));
    ...
    if (scorePercent >= passingScore) {
      currentChapter.done = true;
      const totalChs = modules.reduce((s, m) => s + m.chapters.length, 0);
      const doneChs  = modules.reduce((s, m) => s + m.chapters.filter(c => c.done).length, 0);
      const newProgressPercent = totalChs > 0 ? Math.round((doneChs / totalChs) * 100) : 0;
      const timeSpent = calcTimeSpent();
      setSaving(true); setSavingMsg("Saving assessment result...");
      onProgress(newProgressPercent, timeSpent, scorePercent);
      setTimeout(() => setSaving(false), 1200);
    }
      ...
      {currentChapter.type === 'assessment' && takingAssessment ? (
          ...
          <div style={{ flex:1, overflow:'auto', padding:'32px 20px' }}>
            <div style={{ maxWidth:900, margin:'0 auto' }}>
              {currentChapter.content.body && (
                <div style={{ fontSize:14, color:'var(--t2)', marginBottom:24, lineHeight:1.7 }}>{currentChapter.content.body}</div>
              )}
        ...
        <div className="cv-page">
                ...
                {modules.map((mod, modIdx) => (
                  <div key={modIdx} className="cv-module">
                    ...
                    {mod.chapters.map((ch, chIdx) => {
                      ...
                      return (
                        <div key={chIdx} className={`cv-chapter${isActive ? ' active' : ''}${ch.done ? ' done' : ''}`} onClick={() => goToChapter(modIdx, chIdx)}>
                          ...
                          <div className="cv-chapter-check">{ch.done && '✓'}</div>
                        </div>
                      );
                    })}
                  </div>
              ...
              <div className="cv-content-wrapper">
                    ...
                    return (
                      <div key={block.id} className="cv-activity" style={{ animationDelay:`${idx * 0.05}s` }}>
                        ...
                        {act.type === 'accordion' && act.items && act.items.map((item, itemIdx) => {
                          ...
                          return (
                            <div key={itemIdx} className="cv-accordion-item">
                              <div className={`cv-accordion-question${isOpen ? ' open' : ''}`} onClick={() => setOpenAccordions(prev => ({ ...prev, [key]: !prev[key] }))}>
                                <span className="cv-accordion-icon">💭</span>
                                <span className="cv-accordion-text">{item.q}</span>
                                <svg className={`cv-accordion-chevron${isOpen ? ' open' : ''}`} viewBox="0 0 16 16" fill="var(--purple)"><path d="M4 6l4 4 4-4" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round"/></svg>
                              </div>
                        ...
                        {(act.type === 'checklist' || act.type === 'hotspot') && act.checklist && act.checklist.map((item, itemIdx) => {
                          ...
                          return (
                            <div key={itemIdx} className={`cv-checklist-item${isChecked ? ' checked' : ''}`} onClick={() => setCheckedItems(prev => ({ ...prev, [key]: !prev[key] }))}>
                ...
                {currentChapter.type === 'quiz' && (
                  <div className="cv-quiz">
                    ...
                    {questions.map((q, qIdx) => (
                      <div key={qIdx} className="cv-quiz-question">
                        ...
                        {q.opts.map((opt, optIdx) => {
                          ...
                          return (
                            <div key={optIdx} className={optClass} onClick={() => !submitted && setAnswers(prev => ({ ...prev, [`q${qIdx}`]: optIdx }))}>
                              <div className="cv-quiz-radio" />
                              <div className="cv-quiz-option-text">{opt}</div>
                    ...
                    {submitted && (
                      <div style={{ padding:16, borderRadius:10, background: questions.every((q, idx) => answers[`q${idx}`] === q.ans) ? 'rgba(13,148,136,0.08)' : 'rgba(220,38,38,0.06)', border: questions.every((q, idx) => answers[`q${idx}`] === q.ans) ? '1.5px solid rgba(13,148,136,0.2)' : '1.5px solid rgba(220,38,38,0.2)', marginTop:20 }}>
                        <div style={{ fontSize:15, fontWeight:700, color: questions.every((q, idx) => answers[`q${idx}`] === q.ans) ? 'var(--teal,#0d9488)' : '#dc2626', marginBottom:4 }}>
                          {questions.every((q, idx) => answers[`q${idx}`] === q.ans) ? '✅ Perfect! All answers correct!' : '❌ Some answers are incorrect'}
                        </div>
                        <div style={{ fontSize:12, color:'var(--t2)' }}>
                          {questions.every((q, idx) => answers[`q${idx}`] === q.ans) ? "Great work! You can now proceed." : (
                            <>Attempts: {currentAttempts + 1} / {maxQuizAttempts}{currentAttempts + 1 < maxQuizAttempts && (<div style={{ marginTop:10 }}><button onClick={handleQuizRetry} style={{ padding:'8px 16px', borderRadius:8, border:'none', background:'#dc2626', color:'#fff', fontSize:12, fontWeight:600, cursor:'pointer' }}>🔄 Try Again ({maxQuizAttempts - currentAttempts - 1} left)</button></div>)}</>
                          )}
                        </div>
                      </div>
                    )}
          ...
          <div className="cv-footer">
            <div className="cv-footer-info">{currentChapter.done ? '✓ Completed' : `Chapter ${selCh + 1} of ${currentModule.chapters.length}`}</div>
            {selCh > 0 && (
            ...
            {(currentChapter.type === 'quiz' || currentChapter.type === 'assessment') && !submitted ? (
              ...
              <button className="cv-btn cv-btn-primary" onClick={markComplete}>
                {wasAlreadyCompleted && currentChapter.done ? 'Done' : currentChapter.done ? (isLastChapter ? 'Complete Course' : 'Next Chapter') : (isLastChapter ? 'Complete Course' : 'Mark Complete')}
