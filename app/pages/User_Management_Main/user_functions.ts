...
export function useUserManagement(): UseUserManagementReturn {
  ...
  const fetchUsers = useCallback(() => {
    ...
    console.log('🔵 URL:', url);
    console.log('🔵 API_BASE env:', process.env.NEXT_PUBLIC_API_URL ?? '(not set — falling back to http://localhost)');
    ...
    fetch(url)
      .then(r => {
        ...
        return r.text().then(text => {
          console.log('📥 Raw body preview (first 400):', text.slice(0, 400));
          if (!r.ok) {
            console.error('❌ Non-OK response:', r.status, r.statusText);
            // Parse every clue out of the Laravel error HTML
            const jsonMsg     = text.match(/"message"\s*:\s*"([^"]+)"/)?.[1];
            const exClass     = text.match(/class="exception-title"[^>]*>([^<]+)/)?.[1]?.trim();
            const exMsg       = text.match(/class="exception-message[^"]*"[^>]*>([^<]+)/)?.[1]?.trim();
            const firstFrame  = text.match(/in ([^<"]+\.php)[^\d]*(\d+)/)?.[0];
            const sqlErr      = text.match(/SQLSTATE[^<"]*/)?.[0];
            const columnErr   = text.match(/Unknown column[^<"]*/)?.[0];
            const tableErr    = text.match(/Table '[^']+' doesn't exist[^<"]*/)?.[0];
            const callErr     = text.match(/Call to [^<"]*/)?.[0];

            console.group('🔍 Laravel 500 — extracted details');
            if (jsonMsg)    console.error('💬 message:',      jsonMsg);
            if (exClass)    console.error('🏷️  exception:',   exClass);
            if (exMsg)      console.error('📝 detail:',       exMsg);
            if (sqlErr)     console.error('🗄️  SQL error:',   sqlErr);
            if (columnErr)  console.error('📋 column error:', columnErr);
            if (tableErr)   console.error('📋 table error:',  tableErr);
            if (callErr)    console.error('⚙️  call error:',  callErr);
            if (firstFrame) console.error('📍 location:',     firstFrame);
            if (!jsonMsg && !exClass && !exMsg && !sqlErr && !columnErr && !tableErr && !callErr) {
              console.error('⚠️  Could not extract error details — APP_DEBUG may be false.');
              console.error('👉 Run in your container: tail -20 storage/logs/laravel.log | grep -A3 "exception"');
            }
            console.groupEnd();
      ...
      .then(d => {
        ...
        if (d.success && Array.isArray(d.data)) {
          console.log(`✅ Got ${d.data.length} user(s) from API`);
          console.table(d.data.slice(0, 5));
          setUsers(d.data.map(apiUserToUser));
          setAnimKey(k => k + 1);
        } else {
          console.error('❌ Unexpected response shape. Expected { success: true, data: [] }, got:', d);
