...
import {
  User, UserRole, UserStatus, UserFilters, RoleCardInfo,
...
function RoleCard({ info, count, onClick }: RoleCardProps) {
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      <div className="flex items-center justify-center shrink-0 rounded-lg" style={{ width: 28, height: 28, background: info.iconBg, color: info.iconColor }}>
        {iconSvg}
      </div>
      ...
      <div className="flex items-center" style={{ gap: 4 }}>
        <div className="flex items-center justify-center font-bold rounded-full shrink-0" style={{ fontSize: 9.5, minWidth: 18, height: 18, padding: '0 5px', background: 'var(--purple-lt)', color: 'var(--purple-d)' }}>
          {count}
        </div>
        <svg width="8" height="8" viewBox="0 0 8 8" fill="none" stroke="var(--t4)" strokeWidth="1.8" style={{ opacity: hovered ? 1 : 0, transition: 'opacity .15s' }}>
...
// ─────────────────────────────────────────────────────────────────────────────
//  DropdownFilter  –  filters only commit on "Apply & Close"
// ─────────────────────────────────────────────────────────────────────────────
...
function DropdownFilter({ filters, users, onToggle, onClear, filterCount }: DropdownFilterProps) {
  ...
  // ── Local draft (uncommitted selections) ──────────────────────────────────
  ...
  // Sync draft when dropdown opens so it reflects current applied filters
  ...
  // Count selections in draft (for footer label)
  ...
  function clearDraft() {
    setDraft({ role: new Set(), status: new Set(), company: new Set() });
  }

  function applyAndClose() {
    // Diff draft vs current applied filters and call onToggle for each change
        ...
        >
          ...
          <div className="flex items-start" style={{ padding: '16px 20px 14px', gap: 0 }}>
            ...
            <div className="flex flex-col gap-1 flex-1 min-w-0">
              ...
              {roles.map(r => (
                <OptionRow key={r} label={r} active={draft.role.has(r)} onClick={() => toggleDraft('role', r)} />
              ))}
            </div>
            ...
            <div className="flex flex-col gap-1 flex-1 min-w-0">
              ...
              {statuses.map(s => (
                <OptionRow key={s} label={s} active={draft.status.has(s)} onClick={() => toggleDraft('status', s)} dot={s === 'Active' ? 'dot-g' : 'dot-r'} />
              ))}
            </div>
          ...
          <div className="flex items-center justify-between"
            ...
            <div className="flex items-center gap-2">
              {filterCount > 0 && (
                <button
                  className="flex items-center gap-1.5 font-semibold cursor-pointer transition-all duration-150 rounded-full text-[11.5px]"
              ...
              <button
                className="flex items-center gap-2 cursor-pointer font-semibold transition-all duration-150 rounded-full text-white border-none"
                style={{
                  fontSize: 13, letterSpacing: '.01em',
                  background: 'var(--grad)',
                  fontFamily: "'DM Sans', sans-serif",
                  boxShadow: '0 4px 14px rgba(124,58,237,.40)',
                  padding: '9px 22px',
                }}
...
function Toast({ message, visible }: { message: string; visible: boolean }) {
  return (
    <div
      ...
      style={{
        bottom: 18, right: 20,
        background: 'var(--t1)', padding: '10px 15px',
...
export default function UserManagement() {
  const {
    users, searchQuery, setSearchQuery, filters,
    addOpen, setAddOpen, editOpen, setEditOpen,
    editForm, setEditForm, toast, animKey,
    searchFocused, setSearchFocused, visibleUsers, filterCount,
    ...
    <>
      ...
      <div className="um-page-shell">
        <div className="flex flex-col w-full h-full overflow-hidden" style={{ padding: '18px 24px 16px', background: 'var(--bg)' }}>
            ...
            <div className="flex flex-col overflow-hidden min-h-0 rounded-xl bg-white" style={{ border: '1px solid var(--border)', marginTop: 30 }}>
              ...
              <div className="flex items-center shrink-0" style={{ gap: 10, padding: '10px 14px', borderBottom: '1px solid var(--border)' }}>
                <div
                  ...
                  style={{
                    gap: 8, width: 300, padding: '7px 12px',
              ...
              {/* Pagination */}
              <div className="flex items-center justify-between shrink-0" style={{ padding: '8px 14px', borderTop: '1px solid var(--border)', fontSize: 11, color: 'var(--t3)' }}>
                <span>Showing 1–{visibleUsers.length} of {users.length} entries</span>
                <div className="flex gap-1">
                  {['‹', '1', '2', '3', '›'].map((p, i) => (
                    <button key={i} className="flex items-center justify-center cursor-pointer transition-all duration-150 w-6.25 h-6.25 rounded-[5px] text-[11px] font-medium"
          ...
          {/* ── Popups ── */}
          <AddUserPopup open={addOpen} companies={COMPANY_OPTIONS} onClose={() => setAddOpen(false)} onAdd={handleAddUser} />
