...
export default function CourseCreationWizard({
  ...
  useEffect(() => {
    console.log('[CourseWizard] 🔵 Fetching companies...');
    api.companies.getAll()
      .then(res => {
        console.log('[CourseWizard] 📥 Raw response:', res);
        console.log('[CourseWizard] success:', res.success, '| data:', res.data);
        // Laravel double-wraps: res.data = { success, data: [...] }
        const companies = Array.isArray(res.data) ? res.data : (res.data as any)?.data;
        if (Array.isArray(companies)) {
          console.log('[CourseWizard] 🔍 First company raw object:', companies[0]);
          const mapped = companies
            .filter((c: any) => c.id != null && c.company_name != null)
            .map((c: any) => ({
              id:   c.id,
              name: c.company_name,
              cat:  industryLabel[c.industry_title ?? c.industry] ?? 'Other',
            }));
          console.log('[CourseWizard] ✅ Mapped clients:', mapped);
          setClients(mapped);
        } else {
          console.warn('[CourseWizard] ⚠️ Could not find array in response:', res);
        }
      })
      .catch(err => console.error('[CourseWizard] ❌ Fetch threw:', err));
  }, []);
